...
export default function ApprovalDetail({ navigation, route }) {
    ...
    useEffect(() => {
        ...
        if (params != null && params.data) {
            setData(params.data);
            setStatusColor(params.data.status == 'A' ? 'green' : params.data.status == 'R' : 'red')
        }
    }, []);
                        ...
                        <CardItem>
...

                            {
                                ...
                                <View style={styles.viewBottomNotes}>
                                    <Button primary block rounded onPress={() => doEditTask(data)} style={{ marginTop: 10 }}>
                                        <Text>Approved</Text>
                                    </Button>
                                    <Button danger block rounded onPress={() => doEditTask(data)} style={{ marginTop: 10 }}>
                                        <Text>Rejected</Text>
                                    </Button>
                                </View>
                                :
